...
@observer
export default class AddSelectValue extends FormPropertyConfig<IProps> {
    /** 分组信息编辑模态框实例 */
    ...
    constructor(props: IProps) {
        ...
        this.registerAddSelectValueInstance(this);
    }
    onAdd() {
    ...
    render() {
        ...
        let selectOptionsValue:string = null;
        if (this.props.itemDataType) {
            selectOptionsValue = this.props.itemDataType
        } else {
            const itemvalue = get(formPropertyApp.formProperyElementRef ? formPropertyApp.formProperyElementRef.viewModel.InputDataModel : {},'selectOptionsValue.value') as HlLabeledValue
            if (itemvalue) {
                selectOptionsValue = itemvalue.key
            }
        }
        return (
            <React.Fragment>
                <HLModalContext content={(<React.Fragment>
                    <HLFormContainer
                        ...
                        colCount={1}
                        controls={(selectOptionsValue==='1')?this.createKeyVlaueConfig(this.onAdd.bind(this),formPropertyApp.formPropertyViewModel.selectValueData):this.createAddSelectOptionsValue()}
                    ></HLFormContainer>
                </React.Fragment>)}>
                <HLModal
                    closable={false}
                        ...
                        <Button type="primary" onClick={() => {
                            ...
                            formPropertyApp.formAddSelectValueRef.viewModel.form.validateFields((err,values: PropertyKeyValueFormFields) => {
                                if (!err) {
                                    ...
                                    formPropertyApp.formPropertyViewModel.updatePropertyValue({value:selectOptionsValue,property:'selectOptionsValue',propertyType: type[selectOptionsValue]})
                                    if ((selectOptionsValue&&selectOptionsValue==='1')) {
                                    ...
                                    if (selectOptionsValue&&selectOptionsValue==='3') {
                                        const autoQuery = {};
                    ...
                    onReady={(value) => {
                        const width = 650;
                        formPropertyApp.formSelectValueModalRef = value;
                        formPropertyApp.formSelectValueModalRef.viewModel.okText='保存';
                        formPropertyApp.formSelectValueModalRef.viewModel.width = width;
                    }}>
